...
To elucidate the existence of outliers and thus potential influential points, we visualized the hate crime rate by state. We then generated residuals vs fitted, normal Q-Q, scale-location, and residuals vs leverage plots for a linear regression model that regressed the transformed hate crime rate onto all possible covariates. We looked at the Residuals vs Leverage plot specifically to find any outliers.  Any points outside of Cook's Distance were considered potential influential points.  Any states that were deemed to be an outlier were included in the subsequent analyses, but the same analyses were also run a second time on the dataset excluding the outliers for comparison in order to determine if the outlier was indeed influencing the results of our analyses.
